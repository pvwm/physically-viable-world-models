...
from npm_sim.rigid_ramp_tower import run_simulation
...
class RigidRampTowerSmokeTest(unittest.TestCase):
    def _assert_scene_response(self, result, min_ball_dy: float) -> None:
        self.assertTrue(np.isfinite(result.final_body_poses).all())
        self.assertGreater(result.final_ball_position[1], result.initial_ball_position[1] + min_ball_dy)
    ...
    def test_rigid_ramp_tower_smoke(self) -> None:
        result = run_simulation(viewer="null", num_frames=180, device="cpu")
        self._assert_scene_response(result, min_ball_dy=0.20)

    def test_rigid_superball_smoke(self) -> None:
        ...
        self._assert_scene_response(result, min_ball_dy=0.10)
