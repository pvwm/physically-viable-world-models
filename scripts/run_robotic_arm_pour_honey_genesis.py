"""CLI entry point for the Genesis robot-arm honey-pouring variant.
...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location("robotic_arm_pour_genesis_honey", path)
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    module.configure_honey_variant()
    return module
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default="outputs/robotic_arm_pour_honey_genesis.mp4",
    )
    parser.add_argument(
        "--no-video",
        action="store_true",
    ...
    parser.add_argument(
        "--rebake",
        action="store_true",
        help="Rebuild the settled starting honey cache before running.",
    )
    parser.add_argument(
        "--liquid-vis-mode",
