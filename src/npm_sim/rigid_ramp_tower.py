...
# Fixed rigid scene constants, tuned for one readable passive rigid-body demo.
...
# Tune the all-superball case to keep visible bounce without letting the target pre-collapse at rest.
GROUND_SUPERBALL_BOUNCY = MaterialPreset(density=1200.0, friction=0.18, restitution=0.96)
SUPERBALL_CONTACT_FRICTION = 0.18
SUPERBALL_CONTACT_RESTITUTION = 0.96
SUPERBALL_CONTACT_DAMPING = 0.0
SUPERBALL_ROLLING_FRICTION = 0.0
SUPERBALL_TORSIONAL_FRICTION = 0.0
...
def _superball_shape_cfg(material: MaterialPreset, *, dynamic: bool) -> newton.ModelBuilder.ShapeConfig:
    cfg = _shape_cfg(material) if dynamic else _static_shape_cfg(material)
    cfg.mu = SUPERBALL_CONTACT_FRICTION
    cfg.restitution = SUPERBALL_CONTACT_RESTITUTION
    cfg.kd = SUPERBALL_CONTACT_DAMPING
    cfg.mu_rolling = SUPERBALL_ROLLING_FRICTION
    cfg.mu_torsional = SUPERBALL_TORSIONAL_FRICTION
    return cfg
...
class RampTowerDemo:
    def __init__(
        ...
        if superball_ground_scene:
            ...
            target_labels = ("target_bottom_left", "target_bottom_right")
            for position, label in zip(target_positions, target_labels, strict=True):
                body = builder.add_body(xform=wp.transform(p=position, q=wp.quat_identity()), label=label)
                builder.add_shape_box(
                    body,
                    hx=cube_half,
                    hy=cube_half,
            ...
            top_sphere_body = builder.add_body(
                xform=wp.transform(
                    p=wp.vec3(0.0, TOWER_CENTER_Y, floor_top_z + CUBE_SIZE + sphere_radius),
                    q=wp.quat_identity(),
                ),
                label="target_top_sphere",
            )
            builder.add_shape_sphere(
                top_sphere_body,
            ...
            self.cube_body_indices.append(top_sphere_body)
        else:
            cube_positions = (
                wp.vec3(-cube_half, TOWER_CENTER_Y, tower_base_z),
                wp.vec3(cube_half, TOWER_CENTER_Y, tower_base_z),
